...
*/
{
    ...
    if (status < 0)
    {
    umfpack_di_report_info (NULL, NULL) ;
    ...
    (void) umfpack_di_report_symbolic (Symbolic, NULL) ;
    ...
    (void) umfpack_di_report_numeric (Numeric, NULL) ;

    /* solution */
    /* status = umfpack_di_solve (UMFPACK_A, ia, ja, a, x, b, Numeric, Control, Info) ;*/
    status = umfpack_di_solve (UMFPACK_At, ia, ja, a, x, b, Numeric, NULL, NULL) ;
